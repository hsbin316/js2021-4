...
##### 브라우저 객체 모델
 - 웹 브라우저와 관련된 객체
 - 브라우저 객체 모델[ 교재 311페이지 참고 ]

##### window 객체
 - 웹 페이지 자체
 - 새로운 화면을 열거나 웹 브라우저의 크기를 변경하는 등의 일을 할 수 있다.
 - 경고창과 프롬프트를 제공
 - 경고 함수와 입력 함수
     > alert(<메시지>) 경고창을 출력한다<br>
     prompt(<메시지>,<임시 글자>) 프롬프트를 출력한다
 - 예제 [ 교재 312페이지 참고 ]
...
##### screen 객체
  - screen 객체의 속성
     ...
     acailHeight 실제 화면에서 사용 가능한 너비<br>
...
##### navigator 객체
 - navigator 객체의 속성
     > appCoodeName 웹 브라우저의 코드 이름
     appName 웹 브라우저의 이름
     appVersion 웹 브라우저의 버전
     platform 사용 중인 운영체제의 시스템 환경
     userAgent 웹 브라우저의 전테적인 정보
 - 예제 [ 교재 316페이지 참고 ]

##### 문서 객체 모델
 - 문서 객체 모델
     - html파일을 분석하고 출력하느 방식을 문서 객체 모델이라 한다
     - 문서 객체를 조작하면 태그 내부의 글자,스타일,속성을 변경 할 수 있다
 - 웹 페이지 생성 순서
     - [ 교재 324-327페이지 참고 ]
 - 문서 객체 선택
     - 문서 객체 선택
         > document.getElementById(아이디) 아이디를 사용해 문서 객체를 선택한다<br>
         document.querySelector(선택자) 선택자를 사용해 문서 객체를 선택한다<br>
         document.getElementsByName(이름) name 속성으로 여러 개의 문서 객체를 선택한다<br>
         document.getElementsByClassName(클래스) class 속성으로 여러개의 문서 객체를 선택한다<br>
         document.querySelectorAll(선택자) 선택자로 여러개의 문서객체를 선택한다
 - 문서 객체 조작
     - 문자조작 : 문서 객체 내부의 문자를 조작할 때innerHTML속성을 사용
     - 스타일조작 : 문서 객체의 스타일을 조작할 때는style속성을 사용
     - 속성조작
         > setAttribute(속성 이름,속성 값) 속성을 지정한다<br>
         getAttribute(속성 이름) 속성을 추출한다

##### jQuery 사용 준비
 - 사이트 : http://jquery.com/
 - 다운로드 방법 [ 교재 354 - 357페이지 참고 ]

##### jQuery 객체
 - $함수를 활용
 - JQuery기본사용형태 : $(<매개변수>.메소드(<매개변수>,<매개변수>))
 - 예제 [ 교재 359페이지 참고 ]

##### 문서 객체 선택
 - $()함수를 사용
 - 객체 탐색 메소드
     > parent() 부모 태그를 선택한다<br>
     find() 후손 태그를 찾는다
 - 예제 [ 교재 360-361페이지 참고 ]
...
##### 문서 객체 개별 조작
 ...
 - 선택된 문서 객체 추출
     > get() 선택한 문서 개겣 중 하나를 선택한다
 - 문서 객체 반복 예제 [ 교재 362페이지 참고 ]
 - 선택된 문서 객체 반복 적용
     > each( 선택한 문서 객체에 반복을 적용한다
 - each()메소드의 콜백 함수
     > Array 객체의 forEach()메소드 : [].forEach(function(item,index){});<br>
     jQuery의 each()메소드 : $('h1').each(function(index.item){});
 - 문서 객체 반복 예제 [ 교재 364 페이지 참고 ] 

##### 문서 객체 조작
 - 문자조작
     - 문자 조작 메소드
         > text() html태그 내부의 문자를 조작한다.<br>
         html() html태그 내부의 문자를 조작한다.
     - text()메소드와 html()메소드의 GET 형태 예제 [ 교재 366 페이지 참고 ]
     - text()메소드와 html()메소드의 SET 형태 예제 [ 교재 367 페이지 참고 ]
 - 스타일 조작
     - 스타일 조작 메소드
         > css() 스타일을 조작한다
     - 스타일 조작 예제 [ 교재 369페이지 참고 ]
 - 속성 조작
     - 속성 조작 메소드
         > attr() 속성을 조작한다
     - jQuery를 사용한 속성 조작 예제 [ 교재 370페이지 참고 ]

##### 문서 객체 생성
 - $()함수의 매개 변수에 html 형시의 문자열을 입력
 - 문서 객체 추가 메소드
     > $(< A>.prependTo(< B>)) A를 B안쪽 앞에 추가한다<br>
     $(< A>.appendTo(< B>)) A를 B안쪽 앞에 추가한다<br>
     $(< A>.insertBefore(< B>)) A를 B안쪽 앞에 추가한다<br>
     $(< A>.insertAfter(< B>)) A를 B안쪽 앞에 추가한다
 - 예제  [ 교재 372페이지 참고 ]
